...
@inline function geodesic_eq(u, v, b)
    ComputedGeodesicEquations.@let_unpack u v begin
        ...
        out1 = 0
        out2 = l*v_phi^2*sin_theta + l*v_theta^2
        out3 = 1/2*((b^2 + l^2)*v_phi^2*cos_theta - 4*l*v_r*v_theta)/(b^2 + l^2)
        out4 = -((b^2 + l^2)*v_phi*v_theta*cos_theta + 2*l*v_phi*v_r*sin_theta)/((b^2 + l^2)*sin_theta)
...
@inline function null_constrain(u, v, b)
    ComputedGeodesicEquations.@let_unpack u v begin
        sin_theta = sin(θ)
         ...
         sqrt(b^2*v_phi^2*sin_theta + l^2*v_phi^2*sin_theta + b^2*v_theta^2 + l^2*v_theta^2 + v_r^2)
    end
end
